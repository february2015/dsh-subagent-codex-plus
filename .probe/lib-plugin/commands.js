/**
 * Slash commands for the Codex true-gateway: `/codex-attach` binds the
 * current session to a Codex conversation, `/codex-detach` restores normal
    ...
    return {
        ...
        handler: async (invocation) => {
            ...
            try {
                ...
                return {
                    kind: 'success',
                    text: '已解除直连，本会话恢复 dsh 普通模式。Codex 会话已保留，可随时 `/codex-attach <threadId>` 重新绑定。',
                };
            }
            catch (error) {
                return { kind: 'error', text: error instanceof Error ? error.message : String(error) };
