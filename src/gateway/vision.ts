...
/** One vision-capable chat/completions route. */
export interface VisionBridgeConfig {
  /** OpenAI-compatible base URL, e.g. `https://ocgo.zlxy.sd.cn/v1`. */
  readonly endpoint: string
  /** Bearer API key for the endpoint. */
  readonly apiKey: string
  /** Model name served by the endpoint, e.g. `glm-5.3-flash`. */
...
const DESCRIBE_PROMPT =
  '请用中文描述这张图片：先一句话概括，再分条列出画面内容、可见文字、' +
...
export class VisionBridge {
  constructor(private readonly config: VisionBridgeConfig) {}
  ...
  async describe(data: Uint8Array, mediaType: string): Promise<string> {
 ...
 */
export function readOcgoVisionConfig(tomlPath: string): VisionBridgeConfig | undefined {
  let source: string
  try {
  ...
  if (endpoint === undefined || apiKey === undefined) return undefined
  return {
    endpoint,
    apiKey,
    model: 'glm-5.3-flash',
