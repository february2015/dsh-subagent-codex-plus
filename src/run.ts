 ...
 * quiescence.
 *
 * @module dsh-subagent-codex-plus/run
 */
...
class CodexRunFailure extends Error {
  constructor(
    ...
    super(
      `subagent-codex-plus: ${failureDiagnostic(facts)}`,
...
export function textTask(prompt: readonly ContentBlock[]): string[] {
  if (prompt.length === 0) {
    throw new Error('subagent-codex-plus: the one-shot task must contain only text blocks')
  }
  const texts: string[] = []
  ...
  if (texts.every(text => text.trim().length === 0)) {
    throw new Error('subagent-codex-plus: the one-shot task must not be empty')
  }
  return texts
}
...
export async function startCodexRun(
  ...
  const requestCancel = (): void => {
    ...
    runAbort.abort(new Error('subagent-codex-plus: run cancelled locally'))
    wire.interrupt()
  }
  ...
  try {
    ...
    if (cancelledBeforeCleanup) {
      throw new Error('subagent-codex-plus: request was aborted before run publication')
    }
    try {
